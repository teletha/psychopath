...
public class SevenZipArchiveStreamProvider implements ArchiveStreamProvider {
    ...
    public ArchiveInputStream createArchiveInputStream(String name, InputStream in, String encoding) throws ArchiveException {
        return new SevenZipInputStream(in);
    }
    ...
    public ArchiveOutputStream createArchiveOutputStream(String name, OutputStream out, String encoding) throws ArchiveException {
        return new SevenZipOutputStream(out);
    }
     ...
     * 
     */
    private static class SevenZipInputStream extends ArchiveInputStream {

        private final SevenZFile file;

        public SevenZipInputStream(InputStream input) {
            try (InputStream in = input) {
                ByteArrayOutputStream out = new ByteArrayOutputStream();
                input.transferTo(out);

                this.file = new SevenZFile(new SeekableInMemoryByteChannel(out.toByteArray()));
            } catch (IOException e) {
                throw I.quiet(e);
            }
        }
     ...
     * 
     */
    private static class SevenZipOutputStream extends ArchiveOutputStream {

        private final OutputStream out;

        private final SeekableInMemoryByteChannel bytes = new SeekableInMemoryByteChannel();

        private SevenZOutputFile file;

        public SevenZipOutputStream(OutputStream out) {
            try {
                this.out = out;
                this.file = new SevenZOutputFile(bytes);
            } catch (IOException e) {
                throw I.quiet(e);
            }
        }
